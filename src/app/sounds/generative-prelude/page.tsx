...
import type { PreludeMeasure } from "@/sounds/prelude";

export default function GenerativePreludePage() {
  const ref      = useRef<GenerativePrelude | null>(null);
  const [playing, setPlaying]   = useState(false);
  const [measure, setMeasure]   = useState<number | null>(null);
  const [measures, setMeasures] = useState<readonly PreludeMeasure[]>([]);

  const init = async () => {
    if (ref.current) return;
    const { GenerativePrelude } = await import("@/sounds/generative-prelude");
    ref.current = new GenerativePrelude();
    ref.current.onMeasure = (i) => setMeasure(i);
    setMeasures(ref.current.measures);
  };
  ...
  return (
    <main className="flex flex-col items-center justify-center w-screen h-screen gap-10 bg-stone-950 text-stone-300 font-mono">
      <p className="text-xs tracking-widest text-stone-500">generative prelude · BWV 846 · mm. 1–8, re-shuffled per pass</p>
      ...
      {/* Harmony (pitch set) of current measure — the pool each step is drawn from */}
