...
const PRESETS: { key: DustPreset; label: string; sub: string }[] = [
  { key: "dustPad",       label: "Dust Pad",       sub: "square8 pad · pink noise · reverb 3s" },
  ...
  { key: "cathedralDust", label: "Cathedral Dust", sub: "sine pad · filter LFO · reverb 3s" },
];

export default function GenerativePreludeDustPage() {
  ...
  const handlePlay = async (key: DustPreset) => {
    ...
    const [{ GenerativePrelude }, { createDustVoice }] = await Promise.all([
    ...
    preludeRef.current = new GenerativePrelude(voice);
  ...
  return (
    <main className="flex flex-col items-center justify-center w-screen min-h-screen py-20 gap-3 bg-stone-950 text-stone-300 font-mono">
      <p className="text-xs tracking-widest text-stone-500 mb-2">generative prelude · BWV 846 · dust voices</p>
